...
import { Check, X, Sparkles, Loader2, TrendingUp, ChevronUp, Plus } from 'lucide-react';
      ...
      <div className="flex-1 overflow-hidden flex">
        {COLUMNS.map((col, colIdx) => {
          ...
          return (
            <div
              key={col.id}
              className={`flex-1 flex flex-col min-w-0 ${colIdx < COLUMNS.length - 1 ? 'border-r border-gray-200' : ''}`}
            >
              {/* Column header */}
              ...
              {/* Cards */}
              <div className="flex-1 overflow-y-auto p-3 space-y-2.5 bg-[#f7f8fa]">
                {colAngles.map(angle => (
                  <AngleCard
                    key={angle.id}
                    angle={angle}
    ...
    >
      ...
      <div className="p-4" onClick={onToggle}>
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div
            className={`flex-shrink-0 text-[12px] font-bold px-2 py-0.5 rounded-lg border flex items-center gap-0.5 ${scoreStyle(angle.score)}`}
          >
            {angle.score}
      ...
      <div className="px-4 pb-3 flex items-center gap-1.5">
        {angle.status !== 'approved' && (
          <button
            ...
            className="btn btn-success btn-xs flex items-center gap-1 flex-1 justify-center"
          >
        ...
        {angle.status !== 'rejected' && (
          <button
            ...
            className="btn btn-danger btn-xs flex items-center gap-1 flex-1 justify-center"
          >
            <X size={10} /> Reject
          </button>
        )}
